...
1. Edit `sources.config.json` — append an entry. Fields: `id` (unique), `name`, `type` (`rss`/`api`/`scrape`/`bpc`), `url`, `category` (`tech`/`finance`/`politics`), optional `subcategory`, `enabled`, `useCurl`, `lang`, `locales`, `notes`.
...
For paywall-aware sources, prefer `type: "bpc"` in `sources.config.json`. That adapter shells out to the embedded `tools/bpc-fetch` package by default (or `BPC_FETCH_BIN` if explicitly set) and can optionally use fetched Markdown as excerpts via `bpcFetchFullText: true`.
...
- Don't add Playwright / Puppeteer directly to DailyBrief fetching — normal sources stay light with curl + JSON APIs. Paywall-aware fetching belongs behind the optional external `bpc-fetch` bridge.
